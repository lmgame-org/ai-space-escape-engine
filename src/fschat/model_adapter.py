...
from src.fschat.conversation_game import Conversation, get_conv_template
...
class GrokAdapter(BaseModelAdapter):
    """The model adapter for xAI Grok"""

    def match(self, model_path: str):
        return "grok" in model_path
...
class DashscopeQwenAdapter(BaseModelAdapter):
    """The model adapter for Alibaba Qwen"""

    def match(self, model_path: str):
        return "qwen-max" in model_path or "qwen-plus" in model_path or "qwen-turbo" in model_path
...
register_model_adapter(DashscopeQwenAdapter)
